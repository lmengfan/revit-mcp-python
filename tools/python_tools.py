# -*- coding: utf-8 -*-
"""Python interpreter tools for MCP Server"""
...
# Add lib directory to Python path for config_manager import
...
# Configuration
ALLOW_SYSTEM_ACCESS = ConfigManager.get_env_variable('MCP_PYTHON_ALLOW_SYSTEM_ACCESS', 'false').lower() in ('true', '1', 'yes')
WORKING_DIR = Path(ConfigManager.get_env_variable('MCP_PYTHON_WORKING_DIR', os.getcwd())).absolute()
WORKING_DIR.mkdir(parents=True, exist_ok=True)
DEFAULT_PYTHON_PATH = ConfigManager.get_env_variable('MCP_PYTHON_PATH', sys.executable)
...
def register_python_tools(mcp, revit_get=None, revit_post=None):
    ...
    async def run_python_code(
        ...
        """
        if ctx:
            await ctx.info(f"Executing Python code in {execution_mode} mode...")
    ...
    async def run_python_file(
        ...
        """
        if ctx:
            await ctx.info(f"Executing Python file: {file_path}")
    ...
    async def install_package(
        ...
        """
        if ctx:
            await ctx.info(f"Installing package: {package_name}")
    ...
    @mcp.tool()
    async def read_python_file(file_path: str, max_size_kb: int = 1024, ctx: Context = None) -> str:
        """
    ...
    @mcp.tool()
    async def list_python_files(directory_path: str = "", ctx: Context = None) -> str:
        """
        ...
        """
        if ctx:
            await ctx.info(f"Listing Python files in: {directory_path or 'working directory'}")
    ...
    @mcp.tool()
    async def clear_python_session(session_id: str = "default", ctx: Context = None) -> str:
        """
        ...
        """
        if ctx:
            await ctx.info(f"Clearing session: {session_id}")
    ...
    @mcp.tool()
    async def list_python_sessions(ctx: Context = None) -> str:
        """List all active REPL sessions."""
        if ctx:
            await ctx.info("Listing active Python sessions...")
    ...
    @mcp.tool()
    async def get_python_working_directory(ctx: Context = None) -> str:
        """Get the current working directory for Python operations."""
        if ctx:
            await ctx.info("Getting Python working directory...")
        
        result = f"Python Working Directory: {WORKING_DIR}\n\n"
        result += f"System Access: {'ENABLED' if ALLOW_SYSTEM_ACCESS else 'DISABLED (restricted to working dir)'}\n"
        result += f"Default Python: {DEFAULT_PYTHON_PATH}\n"
        result += f"Platform: {sys.platform}\n"
